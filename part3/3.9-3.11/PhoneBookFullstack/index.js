...
if (process.argv.length === 3) {
  ...
  const url = `mongodb+srv://admin-zoran:${password}@clusterfullstack2023.999pbro.mongodb.net/phonebookApp?retryWrites=true&w=majority`;
  ...
  dbConnection(url).then(() => {
    ...
    getAllPersonsFromAtlas().then((data) => {
      const allData = [...data];
  ...
  dbConnection(url).then(() => {
    console.log("Database connected...");
    addNewPersonToAtlas(personName, personNumber)
      .then((person) => {
        console.log(
